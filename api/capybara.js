...
export default async function handler(req, res) {
  if (req.method === 'GET') {
    try {
      const result = await client.query("SELECT image_url FROM images WHERE animal = 'capybara' ORDER BY RANDOM() LIMIT 1");
      if (result.rows.length > 0) {
        res.status(200).json({ image: result.rows[0].image_url });
      } 
      else {
        res.status(404).json({ error: 'Looks like I have run out of capybara images...' });
      }
    } 
    catch (error) {
